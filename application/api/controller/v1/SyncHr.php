...
class SyncHr extends ApiBase
{
    ...
    public function single($code = 0, $tid = 0, $is_sync = 0)
    {
        ...
        if ($res['code'] == -1) {
            ...
            if (!$DepartmentModel->checkIsCheckLeave($userData)) {
                return $this->jsonReturn_setCache([20002, $res, "用户无须同步"], $cacheKey);
            }
            if ($is_sync && $userData['is_delete'] < 1) {
                OldUserModel::where([['loginname', '=', $code]])->update(['is_delete' => 1, 'modifty_time' => date("Y-m-d H:i:s")]);
            }
