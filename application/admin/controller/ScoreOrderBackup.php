...
class ScoreOrder extends AdminBase
{
    ...
    public function index($filter = [], $status = "0", $page = 1, $pagesize = 15)
    {
        ...
        $lists = OrderModel::alias('t')->field($fields)->join($join)->where($map)->json(['content'])
            ->order('t.operation_time DESC, t.creation_time DESC , t.id DESC')
    ...
    public function detail($id)
    {
        ...
        if (!$data) {
            ...
            if ($data['userInfo']) {
                ...
                $data['userInfo']['avatar'] = $data['userInfo']['imgpath'] ? $avatarBasePath . $data['userInfo']['imgpath'] : $avatarBasePath . "im/default.png";
            }
    ...
    public function good_owners($gid, $time, $pagesize = 20, $filter = ['keyword' => ''])
    {
        ...
        $lists = OrderModel::alias('t')->field($fields)->join($join)->json(['content'])->where($map)
            ->bind('good_num', 0, \PDO::PARAM_INT)->order('t.creator ASC ')
            ->paginate($pagesize, false, ['query' => request()->param()]);
